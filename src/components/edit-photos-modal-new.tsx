...
interface EditPhotosModalNewProps {
  isOpen: boolean;
  onClose: () => void;
...
export function EditPhotosModalNew({ 
  isOpen, 
  onClose, 
  currentPhoto1, 
  currentPhoto2,
  onUpdate 
}: EditPhotosModalNewProps) {
  ...
  const uploadPhoto = async (file: File, photoNumber: 1 | 2): Promise<string | null> => {
    try {
      console.log(`🚀 NEW ALGO: Starting upload for photo ${photoNumber}`);
      
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        console.log('❌ NEW ALGO: No authenticated user');
        return null;
      }

      // Use FIXED filename like About section (avatar approach)
      const fileExt = file.name.split('.').pop();
      const fileName = `photo_${photoNumber}.${fileExt}`;
      const filePath = `${user.id}/${fileName}`;
      
      console.log(`📁 NEW ALGO: Uploading to: ${filePath}`);
      ...
      if (uploadError) {
        console.error(`❌ NEW ALGO: Upload failed:`, uploadError);
        toast({ 
      ...
      console.log(`✅ NEW ALGO: Upload successful`);
      ...
      // Add cache-busting timestamp like About section
      const timestampedUrl = `${publicUrl}?t=${Date.now()}`;
      console.log(`🔗 NEW ALGO: Final URL with cache busting: ${timestampedUrl}`);
      
      return timestampedUrl;
    } catch (error) {
      console.error(`❌ NEW ALGO: Upload error:`, error);
      toast({ 
        title: "Ошибка",
  ...
  const updateProfilePhotos = async (photo1Url: string | null, photo2Url: string | null) => {
    try {
      ...
      console.log(`💾 NEW ALGO: Updating profile photos:`, { photo1Url, photo2Url });

      // Prepare update data - ONLY update fields that have new values
      const updateData: any = {};
      if (photo1Url) {
        updateData.photo_1_url = photo1Url;
        console.log(`📝 NEW ALGO: Will update photo_1_url to: ${photo1Url}`);
      }
      if (photo2Url) {
        updateData.photo_2_url = photo2Url;
        console.log(`📝 NEW ALGO: Will update photo_2_url to: ${photo2Url}`);
      }

      console.log(`📋 NEW ALGO: Final update data:`, updateData);
      ...
      if (profileError) {
        console.error('❌ NEW ALGO: Profile update failed:', profileError);
        throw profileError;
      }

      console.log('✅ NEW ALGO: Profile updated successfully');
      ...
      try {
        ...
        if (participantData) {
          console.log('📋 NEW ALGO: Updating contest participant data...');
          ...
          if (participantError) {
            console.error('❌ NEW ALGO: Participant update failed:', participantError);
          } else {
            console.log('✅ NEW ALGO: Contest participant updated');
          }
        }
      } catch (participantError) {
        console.error('❌ NEW ALGO: Participant update error:', participantError);
      }

      return true;
    } catch (error) {
      console.error('❌ NEW ALGO: Profile update error:', error);
      toast({ 
  ...
  const handleSave = async () => {
    console.log('🔄 NEW ALGO: Starting photo save process...');
    setUploading(true);
    ...
    try {
      ...
      if (photo1File) {
        console.log('⬆️ NEW ALGO: Uploading photo 1...');
      ...
      if (photo2File) {
        console.log('⬆️ NEW ALGO: Uploading photo 2...');
      ...
      if (success) {
        ...
        console.log('🏁 NEW ALGO: Photo save completed successfully');
      }
    } catch (error) {
      console.error('❌ NEW ALGO: Save process failed:', error);
      toast({ 
        title: "Ошибка",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Редактировать фотографии (НОВЫЙ)</DialogTitle>
        </DialogHeader>
