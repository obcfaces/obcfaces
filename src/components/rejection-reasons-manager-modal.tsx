      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh]">
        ...
        <div className="space-y-2 overflow-y-auto max-h-[50vh] pr-2">
          {reasons.map((reason, index) => (
            <div key={reason.key} className="flex items-center gap-2 px-0">
              <Input
                type="number"
                min="1"
                value={reason.sortOrder}
                onChange={(e) => handleSortOrderChange(index, e.target.value)}
                className="w-10 sm:w-12 h-8 text-sm px-1.5 text-center [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              />
              <Input
                value={reason.text}
          ...
          {/* New reason row */}
          <div className="flex items-center gap-2 pt-2 border-t px-0">
            <Input
              type="number"
              min="1"
