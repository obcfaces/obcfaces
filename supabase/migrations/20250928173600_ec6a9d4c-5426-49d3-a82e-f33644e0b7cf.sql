...
-- Create the corrected function to properly return "this week" participants
...
AS $$
  WITH target_week_monday AS (
    SELECT (get_week_monday(CURRENT_DATE) + (weeks_offset * INTERVAL '7 days'))::date as week_monday
  ),
  target_contest AS (
    SELECT 
      wc.id,
    ...
    CROSS JOIN target_week_monday twm
    WHERE wc.week_start_date = twm.week_monday
  ...
  WHERE wcp.is_active = true
    -- For current week (offset 0), only show "this week" participants
    AND (weeks_offset != 0 OR wcp.admin_status = 'this week')
    -- Ensure participant has approved application
    AND EXISTS (
