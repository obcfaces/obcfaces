-- Fix the function to properly handle participants without final_rank
-- and base filtering on actual admin status and week logic
...
AS $function$
  ...
  target_week AS (
    SELECT 
      (monday + (weeks_offset * INTERVAL '7 days'))::DATE as target_start_date,
      (monday + (weeks_offset * INTERVAL '7 days') + INTERVAL '6 days')::DATE as target_end_date
    FROM current_monday
  )
  SELECT 
  ...
  WHERE wcp.is_active = true
    AND wc.week_start_date = tw.target_start_date
