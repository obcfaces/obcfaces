-- Auto-create winner_content entry for weekly contest winner
-- This modifies transition_weekly_contest to automatically create winner entry
...
BEGIN
  ...
  IF dry_run THEN
    ...
    SELECT 
      ...
      average_rating,
      total_votes
    INTO winner_participant_id, winner_user_id, winner_rating, winner_votes
    FROM weekly_contest_participants
    WHERE admin_status = 'this week' AND is_active = true AND deleted_at IS NULL
  ...
  -- 2. CREATE WINNER_CONTENT ENTRY if winner exists and no entry exists yet
  IF winner_participant_id IS NOT NULL AND winner_user_id IS NOT NULL THEN
    -- Check if winner content entry already exists
    IF NOT EXISTS (
      SELECT 1 FROM winner_content 
      WHERE participant_id = winner_participant_id
    ) THEN
      -- Create empty winner content entry for admin to fill in
      INSERT INTO winner_content (
        participant_id,
        user_id,
        payment_proof_url,
      ...
      RAISE NOTICE 'Created winner_content entry for participant % (user %)', winner_participant_id, winner_user_id;
    END IF;
  END IF;
  
  -- 3. SET FINAL RANK for "this week" participants before moving to past
  ...
  -- 4. TRANSITION: "this week" → "past" with CURRENT week interval
  ...
  -- 5. TRANSITION: "next week on site" → "this week" with CURRENT week interval
  UPDATE weekly_contest_participants
  SET 
  ...
  -- 6. TRANSITION: "pre next week" → "next week on site" with CURRENT week interval
  UPDATE weekly_contest_participants
  SET 
