-- Fix the RPC function to select the contest with active participants for "this week"
...
BEGIN
  RETURN QUERY
  WITH contest_with_participants AS (
    SELECT 
      wc.id,
      wc.title,
      wc.status,
      wc.week_start_date,
      wc.week_end_date,
      COUNT(wcp.id) as participant_count
    FROM weekly_contests wc
    LEFT JOIN weekly_contest_participants wcp ON wcp.contest_id = wc.id 
      AND wcp.is_active = true
      AND wcp.participant_status::text = 'this week'
    GROUP BY wc.id, wc.title, wc.status, wc.week_start_date, wc.week_end_date
  ),
  target_contest AS (
    SELECT 
      id,
      ...
      week_start_date,
      week_end_date
    FROM contest_with_participants
    WHERE weeks_offset = 0 AND participant_count > 0  -- For "this week", find contest with participants
    ORDER BY participant_count DESC, week_start_date DESC
    LIMIT 1
  )
  SELECT 
