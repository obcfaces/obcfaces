-- Create a new function to get weekly participants based on admin filtering and week intervals
...
AS $function$
  ...
  target_contest AS (
    SELECT 
      ...
      week_start_date,
      week_end_date
    FROM weekly_contests wc, target_monday tm
    WHERE wc.week_start_date = tm.target_week
  )
  SELECT 
  ...
  WHERE wcp.is_active = true
    AND (
      CASE 
        -- For current week (weeks_offset = 0), show participants with final_rank (this would be filtered by admin "this week" status)
        WHEN weeks_offset = 0 THEN wcp.final_rank IS NOT NULL
        -- For past weeks, show participants from specific week intervals
        ELSE wcp.final_rank IS NOT NULL
      END
    )
