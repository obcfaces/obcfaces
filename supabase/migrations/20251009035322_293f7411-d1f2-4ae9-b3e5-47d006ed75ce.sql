-- Fix get_daily_registration_stats to exclude OAuth users from suspicious count
-- Match the UI filter logic exactly
...
BEGIN
  ...
  registrations AS (
    ...
    SELECT 
      ...
      -- Count suspicious registrations (EXCLUDING users with 'suspicious' role AND OAuth users)
      COUNT(*) FILTER (WHERE 
        ...
        -- 1. CRITICAL: Exclude OAuth users (Google, Facebook)
        AND COALESCE(raw_app_meta_data->>'provider', 'email') NOT IN ('google', 'facebook')
        AND (
          -- 2. Email domain not in whitelist
          (email IS NOT NULL AND SPLIT_PART(email, '@', 2) != ALL(whitelisted_domains))
          OR
          -- 3. Auto-confirmed email < 1 second
          (email_confirmed_at IS NOT NULL AND 
           ABS(EXTRACT(EPOCH FROM (email_confirmed_at - created_at))) < 1)
          OR
          -- 4. Duplicate fingerprint (5+ users with same fingerprint)
          EXISTS (
