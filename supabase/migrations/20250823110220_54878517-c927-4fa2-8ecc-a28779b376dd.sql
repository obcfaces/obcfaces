-- Fix the get_users_who_liked_me function logic
...
AS $function$
  ...
  WITH target_user_info AS (
    SELECT 
      COALESCE(p.display_name, CONCAT(TRIM(COALESCE(p.first_name, '')), ' ', TRIM(COALESCE(p.last_name, '')))) as target_name,
  ...
  SELECT DISTINCT
    ...
    COALESCE(p.display_name, CONCAT(TRIM(COALESCE(p.first_name, '')), ' ', TRIM(COALESCE(p.last_name, '')))) as display_name,
    p.avatar_url,
    p.photo_1_url,
    p.photo_2_url,
    p.country,
  ...
  WHERE l.content_type = 'contest'
    ...
    AND (
      ...
      (l.content_id LIKE 'contestant-card-%' AND 
       (l.content_id = CONCAT('contestant-card-', TRIM(REGEXP_REPLACE(CONCAT(COALESCE(tui.first_name, ''), ' ', COALESCE(tui.last_name, '')), '\s+', ' ', 'g'))) OR
      ...
      (l.content_id LIKE 'contestant-photo-%' AND 
       (l.content_id LIKE CONCAT('contestant-photo-', TRIM(REGEXP_REPLACE(CONCAT(COALESCE(tui.first_name, ''), ' ', COALESCE(tui.last_name, '')), '\s+', ' ', 'g')), '-%') OR
