...
-- Update the get_weekly_contest_participants function to be more secure
-- Only return safe, non-sensitive data for public consumption
...
-- Keep the original function for admin use only (returns all data including sensitive info)
...
-- Grant execute permissions on the public function to everyone
GRANT EXECUTE ON FUNCTION public.get_weekly_contest_participants_public TO anon, authenticated;

-- Grant execute permissions on the admin function only to authenticated users (will be checked by the function itself)
