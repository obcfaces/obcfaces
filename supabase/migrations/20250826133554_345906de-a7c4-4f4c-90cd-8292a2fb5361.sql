...
-- Add application_reference field to contest_applications for easier tracking
ALTER TABLE public.contest_applications 
ADD COLUMN IF NOT EXISTS application_reference text 
GENERATED ALWAYS AS ('APP-' || EXTRACT(YEAR FROM created_at) || '-' || LPAD(EXTRACT(DOY FROM created_at)::text, 3, '0') || '-' || SUBSTRING(id::text, 1, 8)) STORED;

-- Add participant_reference field to weekly_contest_participants
ALTER TABLE public.weekly_contest_participants 
ADD COLUMN IF NOT EXISTS participant_reference text 
GENERATED ALWAYS AS ('PART-' || EXTRACT(YEAR FROM created_at) || '-' || LPAD(EXTRACT(DOY FROM created_at)::text, 3, '0') || '-' || SUBSTRING(id::text, 1, 8)) STORED;
