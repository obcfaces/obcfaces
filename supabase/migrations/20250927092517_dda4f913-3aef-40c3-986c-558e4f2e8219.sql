-- Fix get_weekly_contest_participants_public to include participants from ALL matching contests
...
BEGIN
  ...
  target_contests AS (
    ...
    WHERE EXISTS (
      ...
      WHERE wcp.contest_id = wc.id 
        AND (
          -- For current/next week use participant_status
          (es.status_filter IN ('this week', 'next week') AND wcp.participant_status::text = es.status_filter) OR
          -- For past weeks use admin_status
  ...
  SELECT 
    ...
    -- Return admin_status for past weeks, participant_status for current/future
    COALESCE(
      CASE 
        WHEN wcp.admin_status LIKE 'past week%' THEN wcp.admin_status
        ELSE wcp.participant_status::text 
      END, 
      'this week'
