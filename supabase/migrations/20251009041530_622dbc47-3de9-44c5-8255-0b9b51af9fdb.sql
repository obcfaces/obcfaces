...
-- Fix get_registration_stats_by_type to count ONLY users with 'suspicious' role
-- Match the logic with get_daily_registration_stats
...
BEGIN
  ...
  WITH daily_stats AS (
    SELECT 
      ...
      -- FIXED: Count ONLY users who have been marked with 'suspicious' role
      COUNT(*) FILTER (WHERE 
        EXISTS (
