...
WHERE contest_id = (
  ...
  WHERE status = 'active' 
  AND week_start_date = get_week_monday(CURRENT_DATE)
)
AND user_id = (
  SELECT user_id FROM public.weekly_contest_participants wcp
  JOIN public.weekly_contests wc ON wcp.contest_id = wc.id
  WHERE wc.week_start_date = get_week_monday(CURRENT_DATE - INTERVAL '7 days')
