...
BEGIN
  ...
  SELECT 
    ...
    -- Fix age calculation: current year minus birth year - приводим к integer
    CASE 
      WHEN wcp.application_data->>'birth_year' IS NOT NULL THEN 
        (EXTRACT(YEAR FROM CURRENT_DATE) - (wcp.application_data->>'birth_year')::INTEGER)::INTEGER
      WHEN p.birthdate IS NOT NULL THEN 
        (EXTRACT(YEAR FROM CURRENT_DATE) - EXTRACT(YEAR FROM p.birthdate))::INTEGER
      ELSE p.age
    END as age,
