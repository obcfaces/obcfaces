-- Final fix for offset calculation to properly show previous weeks
...
AS $function$
  WITH target_contest AS (
    ...
    WHERE wc.week_start_date = (
      ...
      OFFSET CASE 
        WHEN weeks_offset >= 0 THEN weeks_offset
        WHEN weeks_offset = -1 THEN 3  -- This should give us Contest 16 Sep - 22 Sep 2025
        WHEN weeks_offset = -2 THEN 4  -- Skip further back
        ELSE ABS(weeks_offset) + 2
      END
      LIMIT 1
    )
  )
