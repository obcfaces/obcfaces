-- Fix get_weekly_contest_participants_admin to return ALL participants (active and inactive)
-- Admins need to see all participants regardless of is_active status
...
AS $function$
  ...
  SELECT 
    ...
    p.display_name,
    COALESCE(p.photo_1_url, wcp.application_data->>'photo1_url') as photo_1_url,
    COALESCE(p.photo_2_url, wcp.application_data->>'photo2_url') as photo_2_url,
    p.avatar_url,
    CASE 
    ...
    COALESCE(ca.status, 'unknown') as application_status,
    COALESCE(wcp.is_active, true) as is_active,
    wcp.admin_status::text as admin_status,
    wcp.week_interval as week_interval,
    TO_CHAR(wcp.created_at, 'YYYY-MM-DD') as status_assigned_date
  ...
  -- REMOVED: WHERE wcp.is_active = true (admins need to see ALL participants)
