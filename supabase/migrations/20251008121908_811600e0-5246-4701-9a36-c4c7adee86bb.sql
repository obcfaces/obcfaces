...
CREATE OR REPLACE FUNCTION public.get_user_auth_data_admin()
 RETURNS TABLE(user_id uuid, email text, auth_provider text, facebook_data jsonb, last_sign_in_at timestamp with time zone, email_confirmed_at timestamp with time zone, created_at timestamp with time zone, user_metadata jsonb)
 LANGUAGE sql
